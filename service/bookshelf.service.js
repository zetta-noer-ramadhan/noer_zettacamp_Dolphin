...
bookshelves.createMany = async (bookshelvesData) => {

    let data = []

    for (let bookshelfData of bookshelvesData) {
        const createdBookshelf = await bookshelfModel
            .create({ name: bookshelfData.name, books: [], genres: [], officer: [] })
            .catch(err => ({ status: 500, err }))
            .then(data => data)

        if (createdBookshelf?.err) return [createdBookshelf.status, createdBookshelf.err]
...
bookshelves.updateOne = async (bookshelfId, bookshelfData) => {
    ...
    if (updatedBookshelf?.err) return [updatedBookshelf.status, updatedBookshelf.err]

    if (!updatedBookshelf) return [200, {
        message: 'no bookshelf fetched',
        detail: 'bookshelf not found'
    }]

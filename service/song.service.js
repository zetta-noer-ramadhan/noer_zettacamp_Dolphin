...
const GetAll = async (filter) => {
    ...
    return await songModel.find(filter).lean().catch((err) => ({ status: 500, err }));
};
...
const UpdateById = async (songId, songData) => {
    ...
    return await songModel.findByIdAndUpdate(validSongId, { ...songData }, { new: true }).lean().catch((err) => ({ status: 500, err }));
};
